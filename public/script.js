...
//purpose: runs the supplied code after 4000 milliseconds (adding delay)
      setTimeout(async () => {
        let initials = "";
        //input validation
        while (initials === "" || initials.length !== 3 || initials.trim().length === 0) {
          //makes the user input only 3 initials
          initials = prompt(`Congratulations! You win!\nPlease enter your 3 initials?`);
        }
...
function startgame() {
  ...
  document.forms.hidden.reset();
    ...
    function fetchAndDisplayHighScores() {
      ...
      xhr.onload = function () {
        if (xhr.status >= 200 && xhr.status < 300) {
          ...
          // Append rows for each high score
          ...
          // Append the table to the highscoreList
          highscoreList.appendChild(table);
        } else {
          console.error('Failed to fetch high scores. Status: ' + xhr.status);
        }
      };
    ...
    // Call the function to fetch and display high scores
